...
class TriggerGenerator:
    ...
    def _generate_triggers(self, list_instruction: List[str]) -> List[str]:
        ...
        score_array = self._eval_triggers(list_instruction, triggers)
        ...
        with tqdm(
            ...
            for current_epoch in progress_bar:
                ...
                with torch.no_grad():
                    ...
                    # Eval Phase Phase 
                    score_array = self._eval_triggers(list_instruction, top_k_triggers)
...
class TriggerValidator:
    ...
    def validate(self, list_instruction: List[str], triggers: List[str]) -> List[str]:
        ...
        for t in tqdm(triggers):
            with torch.no_grad():
                ...
                score_array = self._eval_triggers(list_instruction, resampled_triggers)
                ...
                self._add_logging(list_instruction, t, mean, std, z)

                # Store the trigger and z-score if it meets the criteria
                if z >= z_critical:
    ...
    def run(self, list_instruction: List[str], triggers: List[str]) -> List[str]:

        print(f"Validate triggers for instruction: {list_instruction}")

        triggers_validated = self.validate(list_instruction, triggers)
